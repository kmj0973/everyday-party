    ...
    try {
        ...
        data.products.map((products) => {
            ...
            if (categoryId === products.category) {
                categoryBestCardElement.innerHTML = data.products.map((categoryData) => cardTemplate(categoryData)).join("");
            }

            // 카테고리에 상품을 렌더링
            if (categoryId === products.category) {
                cardAmountElement.innerHTML = `총 ${data.products.length}건`;
                categoryContainerElement.innerHTML = data.products.map((categoryData) => cardTemplate(categoryData)).join("");
            }
        });
    } catch (error) {
